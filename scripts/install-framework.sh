...
# Usage: ./scripts/install-framework.sh [target-dir] [--mode wally|local|rbxm]
...
usage() {
  cat <<'EOF'
Usage: install-framework.sh [OPTIONS] [TARGET_DIR]
...
info() { echo "[framework] $*"; }
ok() { echo "[framework] $*"; }
warn() { echo "[framework] $*" >&2; }

require_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "[framework] '$1' is not on PATH. Run 'aftman install' in the Framework repo." >&2
    exit 1
  fi
}
...
set_wally_dependency() {
  ...
  if grep -qE '^\s*Framework\s*=' "$wally"; then
    sed -i.bak -E "s|^\s*Framework\s*=.*|$framework_line|" "$wally"
    rm -f "$wally.bak"
...
ensure_local_rojo_mount() {
  patch_rojo_project "$1" --framework-src "$REPO_ROOT/src/Framework"
}
...
install_rbxm() {
  require_cmd rojo
  local built="$REPO_ROOT/framework.rbxm"
  (cd "$REPO_ROOT" && rojo build package.project.json -o framework.rbxm)
  mkdir -p "$TARGET/vendor"
  cp "$built" "$TARGET/vendor/framework.rbxm"
  ok "Copied to $TARGET/vendor/framework.rbxm"
  warn "Import: drag vendor/framework.rbxm into ReplicatedStorage in Studio."
}

VER="$(get_version)"
info "Target: $TARGET"
info "Mode: $MODE"
info "Framework version: $VER"

case "$MODE" in
  wally)
    install_wally wally "$VER"
    ok "Installed via Wally."
    echo '  local Framework = require(game:GetService("ReplicatedStorage").Packages.Framework)'
    ;;
  local)
    project="$(find_project_file || true)"
    if [[ -n "${project:-}" && -f "$project" ]]; then
      ensure_local_rojo_mount "$project"
      ok "Linked Framework source via Rojo (no Wally)."
      echo '  local Framework = require(game:GetService("ReplicatedStorage").Framework)'
    else
